...
public class QrCodeProcess {
    ...
    public ChannelResponseInfo process(UrmStoe urmStoe, CupMerchant cupMerchant, ChannelRequestInfo request)
        ...
        NativeTradeRequest entity = packUp(urmStoe, cupMerchant, request);
        String secrekey = cupConfig.getKey();
        NativeTradeResponse response = null;
        response = baseScanPaymentService.addC2bTrade(entity, secrekey);
...
//            0表示成功，非0表示失败此字段是通信标识
        ...
        if (!CupConstant.BUS_SUCCESS.equals(response.getResultCode())) {
            ...
            responseInfo.setCodeUrl(response.getCodeUrl());
            responseInfo.setCodeImgUrl(response.getCodeImgUrl());
        }
        log.info("银联C扫B模式生成二维码结束................");
        return responseInfo;
    }
     ...
     * @return
     */
    private QrChnFlowPa updateChnFlow(QrChnFlowPa chnFlowPa, NativeTradeResponse response) {
        //是否需要保存二维码
        chnFlowPa
                .setBuyerUserId("")
                .setBuyerLogonId("")
                .setAppid("")
                .setTxnCnl(ChannelType.APPSCAN.getCode());
        innerRouteService.updateChnFlow(chnFlowPa);
        return chnFlowPa;

    }

    private NativeTradeRequest packUp(UrmStoe urmStoe, CupMerchant cupMerchant, ChannelRequestInfo request) throws TransferFailedException {
        NativeTradeRequest sdkRequest = new NativeTradeRequest();
